...
const ventClient = new WebhookClient(
    {id : config.venterhook[0], token : config.venterhook[1]});

// Create the webhook that logs the vent message and author.
const revealClient = new WebhookClient(
    {id : config.revealerhook[0], token : config.revealerhook[1]});
...
exports.run = async (client, interaction) => {
  ...
  // Create the modal
  const modal = new ModalBuilder().setCustomId("myVent").setTitle(
      "Post an anonymous vent");
  ...
  client.on(Events.InteractionCreate, async interaction => {
    ...
    if (interaction.customId === "myVent") {
      ...
      try {
        ...
        const ventTitle =
            interaction.fields.getTextInputValue("ventTitleInput");
        ...
        await interaction.editReply({
          embeds : [ {
            author : {
              name : "Processing...",
              icon_url :
                  "https://cdn.discordapp.com/emojis/717875601964269658.gif"
            },
            color : 16763904,
        ...
        if (interaction.fields.getTextInputValue("ventBodyInput")
                .includes("suicid")) {
          ...
          // Set the valuable to true to show in the logs a help message was
          // sent
