exports.run = async (client, message, args, level) => {
  ...
  helpInfoEmbed.addField(
      "`!vent`",
      "**Command used to send messages to #vents**\nYou can either use the command in #send-vents or send a DM to the bot.\nCommand usage: `!vent <message>` for example `!vent Hello`",
      false)
  helpInfoEmbed.addField("\u200B", "\u200B", false);
  helpInfoEmbed.addField(
      "`!ping`",
      "**Command used to check the bot's status.**\nCommand usage: `!ping`",
      false);
  helpInfoEmbed.addField("\u200B", "\u200B", false);
  helpInfoEmbed.addField(
      "Donations",
      "If you want to support Venter financially to cover server costs please donate to https://paypal.me/slemea",
      false);
  helpInfoEmbed.setFooter(
      "If you have any more questions or suggestions contact any Staff members.")
...
exports.run = (client, message, args, level) => {
  ...
  if (!args[0]) {
    ...
    let output = `= Command List =\n[Use ${
        settings.prefix}help <commandname> for details]\n`;
    const sorted = enabledCommands.sort(
        (p, c) =>
            p.help.category > c.help.category ? 1
