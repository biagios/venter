...
function permlevel(message) {
  ...
  const permOrder =
      config.permLevels.slice(0).sort((p, c) => p.level < c.level ? 1 : -1);
...
// settings in enmap should only have *unique* overrides that are different from
// defaults.
