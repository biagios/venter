...
module.exports = async (client, message) => {
  ...
  const prefix = new RegExp(`^<@!?${client.user.id}> |^\\${settings.prefix}`)
...
This command requires level ${container.levelCache[cmd.conf.permLevel]} (${
  ...
  // member so it is supported in DMs) The "level" command module argument will
  ...
  try {
    ...
    logger.log(`${config.permLevels.find(l => l.level === level).name} ${
